...
class ResNet50:
    ...
    def print_dataset_class_info(self):
        ...
        print(f"Classes: {self.test_dataset.classes}")  # ['NORMAL', 'PNEUMONIA']
        print(f"Class mapping: {self.test_dataset.class_to_idx}")  # {'NORMAL': 0, 'PNEUMONIA': 1}
    ...
    def create_confusion_matrix_plot(self, all_labels, all_preds, filename="confusion_matrix_ResNet50.png"):
    ...
    def visualize_cam(self, class_activation_map, width, height, orig_image):
        # 1. Resize CAM to match image dimensions
        heatmap = cv2.resize(class_activation_map, (width, height))
        
        # 2. Apply Color Map (Result is BGR)
        heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
        
        # 3. Overlay: heatmap * 0.3 + original * 0.7
        result = heatmap * 0.3 + orig_image * 0.7
        
        # 4. Cast to uint8 (required for display)
        result = result.astype(np.uint8)

        # 5. Convert BGR (OpenCV default) to RGB (Matplotlib default)
    ...
    def print_result (self, model_path = 'best_model_full_ResNet50.pt', image_folder_path = "chest_xray"):
        
        model, transformation = self.load_and_use_model_ResNet50(model_path)
        model.layer4.register_forward_hook(self.getActivation('final_conv'))
        
        model.eval()
        
        test_dataset  = datasets.ImageFolder(f'{image_folder_path}/test', transform=self.transform)
        test_loader  = DataLoader(test_dataset, batch_size=1, shuffle=False)
        
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        
        for idx, (transformed_img, label) in enumerate(test_loader):
            
            #Move transformed tensor to device for prediction
            transformed_img = transformed_img.to(device)
            
            outputs = model(transformed_img)
            
            cam_map = self.activation['final_conv'].cpu().numpy()
            weight_fc = model.fc.weight.detach().cpu().numpy()
            
            _, pred = torch.max(outputs, 1)
            
            weight_fc = model.fc.weight.detach().numpy()
            output_cam = self.calculate_cam(cam_map, weight_fc, pred.item())
            
            prediction = "PNEUMONIA" if pred.item() == 1 else "NORMAL"
            pred_label = "PNEUMONIA" if label.item() == 1 else "NORMAL"
            print(f'Image index {idx}: Actual {pred_label}, Predicted {prediction}')

            #Get the original file path from the dataset
            img_path, _ = test_loader.dataset.samples[idx]

            #Load the untouched original image
            ...
            plt.title(f"Actual: {pred_label} | Pred: {prediction}")
            plt.axis('off') 
            plt.show()
            
            # 6. Visualize CAM
            # Pass dimensions and the Numpy version of the image
            w, h = original_img_pil.size 
            self.visualize_cam(output_cam, w, h, original_img_np)
            
            # Optional: Break after 5 images so it doesn't run forever
            if idx == 700: 
                break
